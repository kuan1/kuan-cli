...
const getPkg = (options = {}) => {
  const pkg = {
    name: options.name,
    version: defaultPkg.version || '1.0.0',
...
const defaultPkg = getDefaultPkg()
...
const questions = [
  {
    ...
    name: 'name',
    message: '请输入项目英文名称',
    default: defaultPkg.name || path.basename(process.cwd()),
  ...
  },
  {
    ...
    name: 'author',
    message: '请输入作者',
    default: defaultPkg.author || 'kuan1',
    validate: (t) => (t ? true : '请输入'),
  },
  {
    type: 'input',
    name: 'description',
    message: '请输入项目描述',
    default: defaultPkg.description || '没有描述',
    validate: (t) => (t ? true : '请输入'),
  },
  {
    type: 'input',
...
const startGenPkg = async () => {
  ...
  const pkg = { ...defaultPkg, ...getPkg(options) }
